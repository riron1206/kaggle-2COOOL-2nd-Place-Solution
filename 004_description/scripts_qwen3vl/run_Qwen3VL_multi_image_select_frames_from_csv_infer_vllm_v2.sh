...
OUTPUT_DIR="./results/run_Qwen3VL_multi_image_select_frames_from_csv_infer_vllm_v2"
mkdir -p "$OUTPUT_DIR"
...
run_select_and_infer() {
  ...
  python ../src/select_frames_from_csv.py \
    --csv "$CSV" \
...
CSV_DIR="../../002_frame_captioning/results/glm45v_image_frames_infer_perception_vllm_server"
IMAGES_ROOT="../../001_video2frames/gdrive_png/videos"
SKIP_IF_EXISTS=1

for VID_ID in $(seq 0 660); do
  CSV_PATH="$CSV_DIR/$VID_ID.csv"
  IMG_DIR="$IMAGES_ROOT/$VID_ID"
  if [ -f "$CSV_PATH" ] && [ -d "$IMG_DIR" ]; then
    run_select_and_infer "$CSV_PATH" "$IMG_DIR" "$PRE_FRAMES" "$POST_FRAMES" "$SKIP_IF_EXISTS"
  else
    echo "[Warn]: id=$VID_ID (CSV=$CSV_PATH, IMG_DIR=$IMG_DIR)" >&2
  fi
done

python ../src/aggregate_ans_jsons_to_csv.py \
  --root "$OUTPUT_DIR/**/" \
  --output_csv $OUTPUT_DIR/exp004_n_sample4.csv
