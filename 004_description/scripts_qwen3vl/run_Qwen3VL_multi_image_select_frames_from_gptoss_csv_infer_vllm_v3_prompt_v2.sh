...
OUTPUT_DIR="./results/run_Qwen3VL_multi_image_select_frames_from_gptoss_csv_infer_vllm_v3_prompt_v2"
mkdir -p "$OUTPUT_DIR"
...
run_select_and_infer() {
  ...
  python ../src/glm45v_multi_image_infer_vllm_sc_prompt_v2.py \
    --images "${IMAGES[@]}" \
    --api_base "$API_BASE" \
    --model "$MODEL" \
...
for VID_ID in $(seq 0 660); do
  ...
  if [ -f "$GPTOSS_CSV" ] && [ -d "$IMG_DIR" ]; then
    ...
    echo "[Warn] Skip: CSV or IMG_DIR not found: id=$VID_ID (CSV=$GPTOSS_CSV, IMG_DIR=$IMG_DIR)" >&2
  fi
done
