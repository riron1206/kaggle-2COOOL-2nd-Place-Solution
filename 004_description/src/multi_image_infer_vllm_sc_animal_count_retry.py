...
**Completely ignore all text inside the image. Never use image text in your analysis or output.**
...
- Number of animals involved: animals directly involved (e.g., collision or blocking the lane). **Completely ignore all text inside the image. Never use image text in your analysis or output.**
...
def detect_contradictions(
    ...
    """
    Check for simple inconsistencies between counts_json (8 numeric keys) and
    texts_json (2 textual keys), and return a list of inconsistency messages
    (an empty list means no inconsistencies).
    """
    issues: List[str] = []
...
def clamp_counts_cap(d: Dict[str, Any]) -> None:
    """Clip the three target count fields to COUNT_MAX_CAP (in-place)."""
    target_keys = {
        "Number of animals involved",
        "Number of pedestrians involved",
        "Number of vehicles involved (excluding ego-car)",
...
def build_image_urls_from_paths(image_paths: List[str], send_mode: str) -> List[str]:
    if send_mode == "file_url":
        return [to_file_url(p) for p in image_paths]
    image_urls: List[str] = []
...
def run_inference_for_images(
    ...
    sc_mode: str,
    sc_at: str,
    sc_temp: Optional[float],
    sc_top_p: Optional[float],
    sc_top_k: Optional[int],
    json_mode: bool,
) -> Dict[str, Any]:
    image_urls = build_image_urls_from_paths(image_paths, send_mode)

    if sc_at == "all" and n_samples > 1:
        _temp = sc_temp if sc_temp is not None else temperature
        _top_p = sc_top_p if sc_top_p is not None else top_p
        _top_k = sc_top_k if sc_top_k is not None else top_k
        ...
        counts_text, _counts_reason = chat_with_images(
            api_base=api_base,
            api_key=api_key,
            model=model,
            image_urls=image_urls,
    ...
    if sc_at == "all" and n_samples > 1:
        ...
        texts_text, _texts_reason = chat_with_images(
            api_base=api_base,
            api_key=api_key,
            model=model,
            image_urls=image_urls,
    ...
    if n_samples > 1:
        ...
        reconciled_text, _reconciled_reason = chat_with_images(
            api_base=api_base,
            api_key=api_key,
            model=model,
            image_urls=image_urls,
    ...
    if has_empty_values(final_json, JSN_KEYS):
        ...
        for i in range(RETRY_ON_EMPTY_MAX_ATTEMPTS):
            new_rp = min(base_rp + (i + 1) * RETRY_ON_EMPTY_STEP, RETRY_ON_EMPTY_CAP)
            reconciled_text, _ = chat_with_images(
                api_base=api_base,
                api_key=api_key,
                model=model,
                image_urls=image_urls,
    ...
    if final_json.get("Incident window start frame", -1) == -1:
        final_json["Incident window start frame"] = 1
    else:
        try:
            idx = int(
                final_json["Incident window start frame"]
            )
            final_json["Incident window start frame"] = int(
                Path(image_paths[idx]).stem.split(".")[0]
            )
        except Exception:
            final_json["Incident window start frame"] = int(
                final_json.get("Incident window start frame", 1)
            )

    clamp_counts_cap(final_json)
...
def process_csv(
    ...
    out_path = Path(output_csv) if output_csv else in_path

    norm_image_paths = [str(Path(p).resolve()) for p in provided_image_paths]
    if not norm_image_paths:
        raise ValueError("Please specify --images (target images for CSV mode)")
    try:
        target_video_id = int(Path(norm_image_paths[0]).parent.stem)
    except Exception:
        m = re.search(r"\d+", Path(norm_image_paths[0]).parent.stem)
        target_video_id = int(m.group(0)) if m else 0
    ...
    for c in required_cols:
        if c not in fieldnames:
            raise ValueError(f"Required CSV column is missing: {c}")
    ...
    count_cols = required_cols[1:]

    updated = 0
    for row in rows:
        try:
            video_id = _to_int(row.get("video", 0))
        except Exception:
            video_id = 0
        ...
        counts = [_to_int(row.get(col, 0)) for col in count_cols]
        if not any(c > csv_threshold for c in counts):
            print(
                f"[CSV] Skipped due to threshold mismatch: video={video_id} threshold={csv_threshold}"
            )
            continue

        print(f"[CSV] Starting inference video={video_id} images={len(norm_image_paths)}")
        final_json = run_inference_for_images(
            image_paths=norm_image_paths,
            api_base=api_base,
            api_key=api_key,
            model=model,
            send_mode=send_mode,
    ...
    with out_path.open("w", encoding="utf-8", newline="") as f:
        ...
        writer.writeheader()
        for row in rows:
            clamp_counts_cap(row)
            writer.writerow(row)

    print(f"[CSV] Saved: {out_path.resolve()} (updated {updated} rows)")
...
def chat_with_images(
    ...
    print("[Debug] completion tokens:", resp.usage.completion_tokens)
...
def main() -> None:
    ...
    ap.add_argument(
        "--reconcile",
        action="store_true",
        help="Run two-stage inference (PROMPT_COUNT and PROMPT_TEXT) and reconcile when inconsistent",
    )
    ap.add_argument(
        "--tmp_json",
        default=None,
        help="Path to save intermediate JSONs (counts/texts/reconciled and raw content/reasoning_content)",
    )
    ap.add_argument("--output_json", default=OUTPUT_JSON, help="Output JSON path")
    ap.add_argument(
        "--api_base",
        default=DEFAULT_API_BASE,
        help="Base URL for vLLM OpenAI-compatible API (e.g., http://localhost:8000/v1)",
    )
    ap.add_argument(
        "--api_key",
    ...
    ap.add_argument(
        ...
        help="Served model name (e.g., glm-4.5v)",
    )
    ap.add_argument(
        "--send_mode",
        choices=["file_url", "base64"],
        default="file_url",
        help="How to send images: file_url= file:///... or base64= data:image/...;base64,...",
    )
    ...
    ap.add_argument(
        ...
        type=float,
        default=1.0,
        help="Repetition penalty (>1 to suppress repetition)",
    )
    ...
    ap.add_argument(
        "--n_samples", type=int, default=1, help="Number of generations for self-consistency (e.g., 8–16)"
    )
    ap.add_argument(
        "--sc_mode",
    ...
    ap.add_argument(
        ...
        choices=["final", "all"],
        default="final",
        help="Where to apply self-consistency: final=reconcile stage only / all=each stage + reconcile",
    )
    ap.add_argument(
        "--sc_temp",
        type=float,
        default=None,
        help="Override temperature only during self-consistency (falls back to --temperature)",
    )
    ap.add_argument(
    ...
    ap.add_argument(
        "--counts_only",
        action="store_true",
        help="Run only Counts inference and output only Counts keys",
    )
    ap.add_argument(
        "--input_csv",
        type=str,
        default=None,
        help="Input CSV path (enables CSV mode when specified)",
    )
    ap.add_argument(
        "--output_csv",
        type=str,
        default=None,
        help="Output CSV path (defaults to overwriting input_csv if unspecified)",
    )
    ap.add_argument(
        "--csv_threshold",
        type=int,
        default=10,
        help="CSV threshold (infer only rows where any of 4 count columns > threshold)",
    )
    args = ap.parse_args()
    ...
    if args.input_csv:
        process_csv(
            ...
            provided_image_paths=args.images,
            csv_threshold=args.csv_threshold,
    ...
    clamp_counts_cap(ordered_final)
    out_path = Path(args.output_json)
    out_path.write_text(
